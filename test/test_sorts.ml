...
let test_one_sort_adeq_size () =
  ...
  let c1 = S.add_symb db "c1" 0 in
  let c2 = S.add_symb db "c2" 0 in
  let c3 = S.add_symb db "c3" 0 in
  let x = T.Var 5 in
  (* x = c1, x = c2, x = c3 *)
  let clause = {
...
let test_predicate () =
  let prob = Prob.create () in
  let db = prob.Prob.symbols in
  let le = S.add_symb db "le" 2 in
  let succ = S.add_symb db "succ" 1 in
  let zero = S.add_symb db "zero" 0 in
  let x = T.Var 0 in
