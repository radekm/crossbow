...
let lit = Solv.to_lit Solv.Pos
let neg_lit = Solv.to_lit Solv.Neg
...
let test_all_vars_assigned_when_model_found () =
  let s = Solv.create () in
  let vars = Array.init 50 (fun _ -> Solv.new_var s) in
  assert_equal Solv.Ltrue (Solv.solve s [| |]);
  Array.iter
    (fun v ->
      let b = Solv.model_value s v in
      assert_bool "" (b = Solv.Ltrue || b = Solv.Lfalse))
    vars
...
let test_unsat_empty_clause () =
  ...
  assert_equal Solv.Lfalse (Solv.solve s [| |])
  ...
  for h = 0 to holes-1 do
    for p = 0 to pigeons-1 do
      for q = p+1 to pigeons-1 do
        assert_bool
          ""
          (Solv.add_clause s [| neg_lit phs.(p).(h); neg_lit phs.(q).(h); |] 2)
      done
    done
  done;
  phs
...
let test_unsat_with_assumpts () =
  ...
  assert_equal Solv.Lfalse (Solv.solve s assumpts);
  assert_equal Solv.Ltrue (Solv.solve s [| |])
...
let test_sat_with_assumpts () =
  ...
  *)
  ...
  assert_equal Solv.Ltrue (Solv.solve s assumpts)

let suite =
  "Minisat suite" >:::
    [
      "new vars are consecutive ints" >:: test_new_vars_are_consecutive_ints;
